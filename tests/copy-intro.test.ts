 ...
 * ## 왜 문구를 두 번 적는가
 *
 * 아래 `SOURCE` 는 `CONTEXT 5-2` 의 원문을 **손으로 옮겨 적은 사본**입니다.
...
/** CONTEXT 5-2 원문. 앞뒤 빈 줄과 들여쓰기는 비교 전에 걷어냅니다. */
const SOURCE = [
  `
오늘 우리 아이는 어땠나요?

매일 보고 있어도, 매일 조금씩 달라집니다.
그 변화를 혼자 기억하지 않아도 괜찮아요.
`,
  `
기억하지 않아도 괜찮아요.

매일 10초, 오늘 우리가 어땠는지만 남겨두세요.
흐름이 보이고, 병원에서 할 말이 생깁니다.
`,
  `
좋은 날을 하루라도 더.
...
function rendered(index: number): string {
  ...
  const blocks = [page.title, page.lines.join('\n')]
