 ...
 * 의 `.page` 가 grid 로 고정합니다). **그래서 각 그림은 자기 상자의 아래를
 * 기준으로 그려야 합니다** — 바닥선·손·길의 끝이 늘 같은 높이에 오고,
 * 장을 넘길 때 그림만 바뀌고 화면은 움직이지 않습니다.
 *
 * ## 그리지 않는 것
 *
 ...
 * 세 그림의 공통 껍데기.
 *
 * `xMidYMax` — **아래 정렬**입니다. 가운데 정렬이면 그림이 자기 칸 가운데에
 * 떠서 바닥선과 제목 사이에 공백이 고입니다. 아래에 붙이면 그림의 바닥이
 * 곧 제목 위의 선이 되고, 남는 자리는 그림 위쪽으로 가서 방의 여백처럼
 * 읽힙니다.
 *
 * `viewBox` 를 그림마다 다르게 넘깁니다. 하나를 같이 쓰면 상자 안에 이미
 * 바닥선 아래의 빈 자리가 깔려 있어, 아래에 붙는 것이 그 빈 자리까지
 * 포함한 상자가 됩니다. 가로는 셋 다 320 이라 확대 비율이 같습니다.
 */
function Frame({
  box,
  children,
