...
### 자동 백업 — 확인되었고, 막혀도 지켜집니다

**2026-09-13 사용자 확인: 두 경로 모두 파일이 만들어집니다.** 기록 5탭을
마치면 태그 화면이 뜨기 전에 json 이 자동으로 떨어지고, 다시 들어가
`백업 파일 받기` 를 눌러도 json 이 떨어집니다.

> ⚠️ **어느 기기에서 본 것인지는 확인되지 않았습니다.** 데스크톱
> 브라우저라면 iOS Safari 의 질문은 그대로 남습니다. 아래 구조는 어느
> 쪽이든 성립하므로 고칠 것은 없지만, 이 문장을 "iOS 에서 확인됨"으로
> 읽지는 마세요.

**자동 백업이 되는 기기에서는 파일이 두 개 생깁니다** (자동 1 + 안전망 1).
앱이 자동 내려받기의 성공 여부를 알 수 없어서 생기는 결과이고, 아래가
그 이유입니다. 줄이려면 안전망을 없애야 하는데, 그러면 자동이 막힌
기기에서 백업이 하나도 없게 됩니다.
...
아직 **아이폰에서** 확인되지 않았다면 그것만 남습니다. 막힌다면
`/pet/backup` 의 내보내기도 같이 막힌 것이고, 그때는 백업 기능 전체를
다시 설계해야 합니다.
