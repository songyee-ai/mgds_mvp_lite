...
- 마지막 커밋: `789f62d` 아이콘 — 브라우저 탭과 홈 화면
...
| 파일 | 배경 | 쓰이는 곳 |
|---|---|---|
| `favicon-32.png` · `favicon-16.png` | **투명** | 브라우저 탭 |
| `apple-touch-icon.png` (180) | **불투명 초록** | iOS 홈 화면 |
| `icon-192.png` · `icon-512.png` | **불투명 초록** | Android · 매니페스트 |

**홈 화면 아이콘에 투명을 두면 안 됩니다.** iOS 가 모서리를 스스로 깎기
때문에 투명한 자리는 검게 남거나 잘립니다. Android 의 maskable 도 같은
이유로 내용이 가운데 80% 안에 있어야 하고, 지금은 약 61% 라 여유가
있습니다.
...
아이콘을 다시 만들 때 쓴 값 — 원본은 1254×1254 RGBA 의 둥근 배지이고,
원의 중심 `(625, 626)`, 실한 반지름 **420**, 그 바깥 약 15px 이 부드러운
헤일로입니다.

| | |
|---|---|
| 불투명 아이콘 | 중심에서 한 변 **1050** 을 잘라 냄 (내용이 아이콘 폭의 약 61%) |
| 헤일로 제거 | 반지름 **415** 로 클립하고 나머지를 `#788C6E` 로 채움 |
| 파비콘 | 한 변 **870**, 투명 유지 (작은 크기에선 부드러운 가장자리가 나음) |

**헤일로를 지우지 않으면 평평한 초록 위에 옅은 고리로 남습니다.** 원 속
초록과 채우는 초록이 같은 색이라 클립 자국은 보이지 않고, 잘려 나가는
것은 헤일로뿐입니다. 채우는 색이 원 속 색과 **2~3/255 만 달라도** 고리가
다시 보입니다 — 실제로 `#798B6F` 로 두었을 때 그랬습니다.
